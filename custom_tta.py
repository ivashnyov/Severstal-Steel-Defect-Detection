...
def fliplr_image2label(model: nn.Module, data) -> Tensor:
...
def fliplr_image2mask(model: nn.Module, image : Tensor) -> Tensor:
    ...
    """
    output = model(image)
    output_flipr = model(F.torch_fliplr(image))
    total_output = {}
    one_over_2 = float(1.0 / 2.0)
    for key in output_flipr.keys():
        total_output[key] = (output_flipr[key] + output[key]) * one_over_2
    return total_output
...
def d4_image2mask(model: nn.Module, image : Tensor) -> Tensor:
    ...
    for aug, deaug in zip([F.torch_rot90, F.torch_rot180, F.torch_rot270],
        ...
        output_aug = model(aug(image))
        for key in output.keys():
            output[key] = output[key] + deaug(output_aug[key])
    ...
    one_over_8 = float(1.0 / 8.0)
    for key in output.keys():
        output[key] = output[key] * one_over_8
